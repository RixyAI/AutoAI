...
This page lists all [🧩 Components](./components.md) and [⚙️ Protocols](./protocols.md) they implement that are natively provided. They are used by the AutoAI agent.
...
It is possible to use your own self-hosted Stable Diffusion WebUI with AutoAI. **Make sure you are running WebUI with `--api` enabled.**
...
Allows agent to search the web. Google credentials aren't required for DuckDuckGo. [Instructions how to set up Google API key](../../AutoAI/configuration/search.md)
