# Creating Challenges for AutoAI

🏹 We're on the hunt for talented Challenge Creators! 🎯

Join us in shaping the future of AutoAI by designing challenges that test its limits. Your input will be invaluable in guiding our progress and ensuring that we're on the right track. We're seeking individuals with a diverse skill set, including:
...
Are you ready to play a pivotal role in AutoAI's journey? Apply now to become a Challenge Creator by opening a PR! 🚀


# Getting Started
Clone the original AutoAI repo and checkout to master branch
...
Go to https://github.com/RixyAI/AutoAI/blob/master/autoai/tests/integration/agent_factory.py
...
def kubernetes_agent(
    ...
    command_registry.import_commands("autoai.commands.file_operations")
    command_registry.import_commands("autoai.app")
...
from autoai.commands.file_operations import read_file, write_to_file
...
def test_information_retrieval_challenge_a(kubernetes_agent, monkeypatch) -> None:
    ...
    monkeypatch.setattr("autoai.utils.session.prompt", lambda _: next(gen))
