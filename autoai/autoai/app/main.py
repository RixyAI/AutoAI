...
from autoai.agent_factory.configurators import configure_agent_with_state, create_agent
from autoai.agents.agent_manager import AgentManager
from autoai.agents.prompt_strategies.one_shot import AssistantThoughts
from autoai.app.config import (
    AppConfig,
    ConfigBuilder,
...
async def run_auto_ai(
    ...
    if not agent:
        ...
        while task.strip() == "":
            task = clean_input(
                "Enter the task that you want AutoAI to execute,"
...
async def run_interaction_loop(
    ...
    def graceful_agent_interrupt(signum: int, frame: Optional[FrameType]) -> None:
        ...
        if cycles_remaining in [0, 1]:
            ...
            logger.warning(
                "Press Ctrl+C again if you want to stop AutoAI immediately."
            )
