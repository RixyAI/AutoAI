...
from autoai.app.config import AppConfig
from autoai.app.utils import is_port_free
...
class AgentProtocolServer:
    ...
    async def start(self, port: int = 8000, router: APIRouter = base_router):
        ...
        app = FastAPI(
            title="AutoAI Server",
            description="Forked from AutoAI Forge; "
        ...
        logger.info(f"AutoAI server starting on http://localhost:{port}")
    ...
    def _get_task_llm_provider(self, task: Task, step_id: str = "") -> MultiProvider:
        ...
        if task.additional_input and (user_id := task.additional_input.get("user_id")):
            _extra_request_headers["AutoAI-UserID"] = user_id
