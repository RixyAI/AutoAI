...
def get_bulletin_from_web():
    try:
        response = requests.get(
            "https://raw.githubusercontent.com/RixyAI/AutoAI/master/autoai/BULLETIN.md"  # noqa: E501
        )
...
def vcs_state_diverges_from_master() -> bool:
    ...
    """
    paths_we_care_about = "autoai/autoai/**/*.py"
    try:
        repo = Repo(search_parent_directories=True)
        ...
        for remote in repo.remotes:
            if remote.url.endswith(
                tuple(
                    ...
                    for path in (
                        f"RixyAI/{n}.git" for n in ("AutoAI", "auto-ai")
                    )
                )
            ):
                og_remote = remote
            ...
            # Original AutoAI remote is not configured: assume local codebase diverges
            return True

        master_branch = og_remote.refs.master
        with contextlib.suppress(StopIteration):
            next(repo.iter_commits(f"HEAD..{master_branch}", paths=paths_we_care_about))
            # Local repo is one or more commits ahead of OG AutoAI master branch
            return True
...
def get_latest_bulletin() -> tuple[str, bool]:
    ...
    news_header = Fore.YELLOW + "Welcome to AutoAI!\n"
    if new_bulletin or current_bulletin:
        news_header += (
            "Below you'll find the latest AutoAI News and feature updates!\n"
            "If you don't wish to see this message, you "
            "can run AutoAI with the *--skip-news* flag.\n"
        )
...
def print_python_version_info(logger: logging.Logger):
    if sys.version_info < (3, 10):
        logger.error(
            ...
            "parts of AutoAI with this version. "
