"""
This is a minimal file intended to be run by users to help them manage the autoai projects.
...
def setup():
    ...
    if install_error:
        click.echo(
            click.style(
                "\n\n🔴 If you need help, please raise a ticket on GitHub at https://github.com/RixyAI/AutoAI/issues\n\n",
                fg="magenta",
                bold=True,
            )
        )
...
def start(agent_name: str, no_setup: bool):
    ...
    agent_dir = os.path.join(
        script_dir,
        f"agents/{agent_name}"
        if agent_name not in ["autoai", "forge"]
        else agent_name,
    )
...
def list():
    ...
    try:
        ...
        if os.path.isdir("./autoai"):
            agents_list.append("autoai")
        if agents_list:
